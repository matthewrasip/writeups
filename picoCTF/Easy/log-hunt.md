# Log Hunt
#### debug info: [u:953389 e: p: c:527 i:297146]
...
# Approach
Looking at the file, we see that there are logs with a tag labelled `INFO FLAGPART`. Using grep, we can search for each line containing the above text: `grep "INFO FLAGPART" server.log`.
```
...
```
Ignoring the duplicates we can determine that the flag is `picoCTF{us3_y0urlinux_sk1lls_cedfa5fb}`